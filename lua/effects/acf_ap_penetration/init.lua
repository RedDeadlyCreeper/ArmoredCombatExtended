...
function EFFECT:Init( data )
	...
	if IsValid(self.AmmoCrate) then
	...
	if self.Id and not RoundTypesIgnore[self.Id] and ((IsValid(TraceEntity) and not EntityFilter[TraceEntity:GetClass()]) or SurfaceTr.HitWorld) then
		...
		-- RDC please, avoid doing elseif chains
...
function EFFECT:Concrete( SmokeColor )
	...
	local DustSpeed = 50
	for _ = 1, ParticleCount do
		local Dust = self.Emitter:Add("particles/smokey", self.Origin)
	...
	--[[
		...
		for _ = 1, 3 do
			...
			if Dust then
				Dust:SetVelocity(self.DirVec * DustSpeed * (self.Caliber + 1))
				DustSpeed = DustSpeed + 50
				--			Dust:SetVelocity(VectorRand() * math.random(20, 30 * Energy))
				Dust:SetLifeTime(0)
				...
				Dust:SetStartAlpha(200)
				Dust:SetEndAlpha(20)
				Dust:SetStartSize(8 * (self.Caliber + 1))
				Dust:SetEndSize(25 * (self.Caliber + 1) * (DustSpeed / 50))
				Dust:SetRoll(math.Rand(150, 360))
				Dust:SetRollDelta(math.Rand(-0.2, 0.2))
				Dust:SetAirResistance(150 / self.Caliber)
	...
	for _ = 1, ParticleCount do
		...
		if Debris then
			Debris:SetVelocity((self.HitNorm + VectorRand() * 0.3) * -30 * Energy)
			Debris:SetLifeTime(0)
			Debris:SetDieTime(math.Rand(1, 2) * (Energy / 3))
			Debris:SetStartAlpha(255)
			Debris:SetEndAlpha(255)
...
function EFFECT:Wood( SmokeColor )
	...
	if Dust then
		...
		Dust:SetStartAlpha(200)
		Dust:SetEndAlpha(75)
		Dust:SetStartSize(10 * self.Caliber)
		Dust:SetEndSize(35 * self.Caliber)
		Dust:SetRoll(math.Rand(150, 360))
		Dust:SetRollDelta(math.Rand(-0.2, 0.2))
		Dust:SetAirResistance(15)
...
function EFFECT:Glass( SmokeColor )
	...
	if Dust then
		...
		Dust:SetStartAlpha(200)
		Dust:SetEndAlpha(75)
		Dust:SetStartSize(15 * self.Caliber)
		Dust:SetEndSize(45 * self.Caliber)
		Dust:SetRoll(math.Rand(150, 360))
		Dust:SetRollDelta(math.Rand(-0.2, 0.2))
		Dust:SetAirResistance(15)
		Dust:SetColor(SmokeColor.r * 0.9, SmokeColor.g * 0.9, SmokeColor.b * 0.9)
...
--	end
	...
	Flash = self.Emitter:Add("effects/ar2_altfire1b", self.Origin + self.HitNorm * -5)
