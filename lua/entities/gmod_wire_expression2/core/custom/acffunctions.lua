...
do
	...
	-- Returns an array of all fuel tanks linked to the entity.
	e2function array entity:acfGetFuelTanks()
		...
		return table.Copy(this.FuelLink)
	end
...
do
	...
	-- Returns an array of all crew seats linked to the entity
	e2function array entity:acfGetCrew()
		if not (isGun(this) or isEngine(this)) then return {} end
		if restrictInfo(self, this) then return {} end
		if not next(this.CrewLink) then return {} end

		return table.Copy(this.CrewLink)
	end
	...
	e2function array entity:acfGetAmmoCrates()
		...
		return table.Copy(this.AmmoLink)
	end
