...
ACF.AmmoBlacklist.HEATFS = { "C","MG", "Al","HMG" ,"RAC", "SC","ATR" , "MO" , "RM", "SL", "GL", "HW", "SC", "BOMB" , "GBU", "ASM", "AAM", "SAM", "UAR", "POD", "FFAR", "ATGM", "ARTY", "ECM", "FGL"}
...
function Round.convert( Crate, PlayerData )
	...
	Data.SlugDragCoef = ((SlugFrAera/10000)/Data.SlugMass)*2
...
function Round.detonate( Index, Bullet, HitPos, HitNormal )
	...
	Bullet.FuseLength = 0.005 + 10/((Bullet.Flight + Bullet.Flight:GetNormalized() * Bullet.SlugMV * 39.37):Length()*0.0254)
	Bullet.Pos = HitPos
	...
	if ACF_Check( Target ) then
		...
		if Bullet.Detonated then
			...
			if HitRes.Overkill > 0 then
				...
				ACF_Spall( HitPos , Bullet.Flight , Bullet.Filter , (Energy.Kinetic*(HitRes.Loss)+0.1)*64 , Bullet.CannonCaliber , Target.ACF.Armour , Bullet.Owner , Target.ACF.Material) --Do some spalling
